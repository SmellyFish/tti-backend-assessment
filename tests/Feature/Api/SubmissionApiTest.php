...
class SubmissionApiTest extends TestCase
{
    ...
    public function test_index_returns_paginated_submissions_newest_first(): void
    {
        ...
        $oldest = $this->createProSubmissionWithAnswers($patient, $instrument, $questions, now()->subHours(2), 2, true, 'Old note');
        $middle = $this->createProSubmissionWithAnswers($patient, $instrument, $questions, now()->subHour(), 3, false, 'Middle note');
        $newest = $this->createProSubmissionWithAnswers($patient, $instrument, $questions, now(), 5, true, 'Newest note');
    ...
    {
        [$patient, $instrument, $questions] = $this->seedProPatientWithInstrument();
        $submission = $this->createProSubmissionWithAnswers($patient, $instrument, $questions, now(), 4, true, 'Notes');
    ...
    {
        [$patientOne, $instrument, $questions] = $this->seedProPatientWithInstrument();
        $patientTwo = Patient::query()->create([
            'name' => 'Other Patient',
            'date_of_birth' => '1978-09-17',
            'mrn' => 'MRN-OTHER-001',
        ]);
        $submission = $this->createProSubmissionWithAnswers($patientTwo, $instrument, $questions, now(), 2, false, '');
    ...
    {
        [, $instrument, $questions] = $this->seedProPatientWithInstrument();
